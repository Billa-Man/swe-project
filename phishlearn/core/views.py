...
def dashboard(request):
    ...
    if user_profile.user_type == 'employee':
        ...
        for attempt in quiz_attempts:
            cid = attempt.quiz.course.id
            if attempt.score >= 70:
                completed_course_ids.add(cid)
            elif cid not in completed_course_ids:
                in_progress_course_ids.add(cid)
        in_progress_course_ids -= completed_course_ids
        completed_count = len(completed_course_ids)
        in_progress_count = len(in_progress_course_ids)
        not_started_count = max(0, total_courses - completed_count - in_progress_count)
...
def take_quiz(request, quiz_id):
    ...
    if request.method == 'POST':
        score, total = 0, questions.count()
        for question in questions:
            selected = request.POST.get(f'question_{question.id}')
            if selected and Choice.objects.get(id=selected).is_correct:
                score += 1
        pct = int((score / total) * 100)
        QuizAttempt.objects.create(user=request.user, quiz=quiz, score=pct)
        QuizAssignment.objects.filter(
            user=request.user, quiz=quiz, status='pending'
        ).update(status='completed')
        Notification.objects.create(
            user=request.user,
            message=f"You've completed the quiz: {quiz.title} with a score of {pct}%",
            link=reverse('course_detail', args=[quiz.course.id])
        )
        messages.success(request, f'Quiz completed! Your score: {pct}%')
...
def manage_employees(request):
    if request.user.userprofile.user_type != 'it_owner':
        ...
        return redirect('dashboard')
    if request.method == 'POST':
        group = EmployeeGroup.objects.create(name=request.POST.get('group_name'), it_owner=request.user)
        for email in request.POST.get('employee_emails', '').split(','):
            email = email.strip()
            try:
    ...
    return render(request, 'core/manage_employees.html', {'groups': groups})
...
def list_employees(request):
    ...
    employees = User.objects.filter(userprofile__user_type='employee', is_staff=False)
    if search_query:
    ...
    paginator = Paginator(employees.distinct(), 20)
...
def create_employee(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.username, pwd = user.email, form.cleaned_data['password']
...
def create_group(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            group.save()
            for email in form.cleaned_data['employee_emails'].split(','):
                email = email.strip()
...
def manage_courses(request):
    if request.user.userprofile.user_type != 'site_admin':
        messages.error(request, 'Unauthorized access')
        return redirect('dashboard')
    if request.method == 'POST':
...
def manage_templates(request):
    ...
    return render(request, 'core/manage_templates.html', {'templates': templates})
...
def login_dashboard(request):
    ...
    attempts = response.json() if response.status_code == 200 else []
    return render(request, 'core/login_dashboard.html', {'login_attempts': attempts})
...
def courses_list(request):
    ...
    completed_ids, in_progress_ids = set(), set()
    for at in attempts:
        cid = at.quiz.course.id
        if at.score >= 70:
            completed_ids.add(cid)
        elif cid not in completed_ids:
            in_progress_ids.add(cid)
    data = []
    for course in courses:
        status, date, score = ('not_started', None, None)
        if course.id in completed_ids:
            latest = attempts.filter(quiz__course=course, score__gte=70).latest('completed_at')
            status, date, score = ('completed', latest.completed_at, latest.score)
        elif course.id in in_progress_ids:
            latest = attempts.filter(quiz__course=course).latest('completed_at')
            status, date, score = ('in_progress', latest.completed_at, latest.score)
        data.append({'id': course.id, 'title': course.title, 'description': course.description, 'content': course.content, 'completion_status': status, 'completion_date': date, 'score': score})
    return render(request, 'core/courses_list.html', {'courses': data})

@login_required
def course_view(request, course_id):
    course = get_object_or_404(Course, id=course_id)
    attempts = QuizAttempt.objects.filter(user=request.user, quiz__course=course)
    if attempts.filter(score__gte=70).exists():
        latest = attempts.filter(score__gte=70).latest('completed_at')
        status, date, score = ('completed', latest.completed_at, latest.score)
    elif attempts.exists():
        latest = attempts.latest('completed_at')
        status, date, score = ('in_progress', latest.completed_at, latest.score)
    else:
        status, date, score = ('not_started', None, None)
    if request.method == 'POST':
        ...
        if action == 'complete':
            if not quizzes.exists():
                quizzes = [Quiz.objects.create(course=course, title=f"{course.title} Quiz", description=f"Quiz for {course.title}")]
            for q in quizzes:
                QuizAttempt.objects.create(user=request.user, quiz=q, score=100)
            Notification.objects.create(user=request.user, message=f"You've completed the course: {course.title}", link=reverse('course_view', args=[course.id]))
            messages.success(request, 'Course marked as complete')
            return redirect('course_view', course_id=course.id)
        elif action == 'in_progress':
            if not quizzes.exists():
                quizzes = [Quiz.objects.create(course=course, title=f"{course.title} Quiz", description=f"Quiz for {course.title}")]
            QuizAttempt.objects.create(user=request.user, quiz=quizzes[0], score=10)
            messages.success(request, 'Course marked as in progress')
            return redirect('course_view', course_id=course.id)
    return render(request, 'core/course_view.html', {'course': course, 'completion_status': status, 'completion_date': date, 'score': score})

# Campaign Views (Incoming Change)
...
class SendingProfilesView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, self.template_name, {'profiles': profiles, 'form_fields': self.get_form_fields()})
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            logger.error(f"Unable to create sending profile: {e}")
            return None
        return render(request, self.template_name, {'profiles': get_sending_profiles() or [], 'form_fields': self.get_form_fields()})
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if not data['password']:
                ...
                data['password'] = existing.get('password', '') if existing else ''
...
class DeleteSendingProfileView(View):
    ...
    def delete_profile(self, request):
        try:
            ...
            messages.error(request, f"Error: {str(e)}")
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({'status': 'error', 'message': 'Failed to delete profile'}, status=400)
        return redirect('sending_profiles')
...
class LandingPagesView(View):
    ...
    def get(self, request):
        ...
        return render(request, self.template_name, {'pages': pages, 'form_fields': form_fields})
...
def reset_api_key_view(request):
    ...
    if request.method == 'POST':
        ...
        if result and result.get('success'):
            messages.success(request, 'API Key Reset! Update .env and restart.')
...
def gophish_analytics(request):    
    return render(request, 'core/gophish_analytics.html', {'page_title': 'Gophish Campaign Analytics'})
...
def gophish_campaigns(request):
    return render(request, 'it_owner/gophish_campaigns.html', {'page_title': 'GoPhish Campaigns'})
...
def quizzes(request):
    ...
    quizzes = Quiz.objects.all()
    employees = User.objects.filter(userprofile__user_type='employee')
    return render(request, 'core/quizzes.html', {'quizzes': quizzes, 'employees': employees})
